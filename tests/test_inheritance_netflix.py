...
from tests.selectors import EXPECTED_TITLE, URL, XPATH_ESTATE_BUTTON, APPLICANT_DATA_NAV_ID, FIRST_FORM_NAME_FIELD, \
    ...
    XPATH_DOWNLOAD_BUTTON, XPATH_INHERITANCE_BUTTON


def test_inheritance_netflix(browser):
    browser.get(URL)
    assert browser.title == EXPECTED_TITLE

    inheritance_button = browser.find_element(By.XPATH, XPATH_INHERITANCE_BUTTON)
    inheritance_button.click()
    ...
    dropdown_menu_defendant.select_by_visible_text('Wyłącz netflixa')
